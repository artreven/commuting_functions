...
class MergeConflictException(Exception):
    def __str__(self):
        return "Key already in dictionary and has a different value"
...
class DiscreteFunction(object):
    ...
    def __init__(self, domain, dict_f={}, arity=None):
        ...
        if not all(len(input_) == arity for input_ in dict_f.keys() if input_):
            raise Exception, 'Not all inputs have same length.'
        self._dict_f = dict_f
        self.arity = arity
        self.domain = domain
